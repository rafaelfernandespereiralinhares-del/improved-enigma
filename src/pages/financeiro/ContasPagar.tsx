...
import { Plus, Trash2, CalendarIcon, CheckCircle, AlertCircle } from "lucide-react";
...
export default function ContasPagar() {
    ...
    const handleImport = async (importedData: any[]) => {
        if (!empresaId || !lojaId) {
            toast.error("Erro: ID da empresa ou loja não encontrado.");
            return;
        }
        try {
            await importService.importContasPagar(importedData, empresaId, lojaId);
    ...
    const handleSave = async () => {
        ...
        if (!newItem.fornecedor || !newItem.valor) {
        ...
        try {
            const { error } = await supabase.from('contas_pagar').insert({
                ...
                valor: parseFloat(newItem.valor.replace(',', '.')), // Basic sanitize
            ...
            setNewItem({ fornecedor: "", valor: "", vencimento: new Date(), status: "Pendente" }); // Reset
            fetchData();
        } catch (error) {
            console.error(error);
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
                        ...
                        <DialogContent>
                            <DialogHeader>
                                <DialogTitle>Novo Contas a Pagar</DialogTitle>
                                <DialogDescription>Adicione uma nova despesa manualmente.</DialogDescription>
                            </DialogHeader>
                    ...
                    <DataImportButton onImport={handleImport} label="Contas a Pagar" />
                </div>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-3">
                ...
                <div className="rounded-xl border bg-card text-card-foreground shadow p-6">
                    ...
                    <div className="text-2xl font-bold text-green-500">
                        R$ {data.filter(i => i.status === 'Pago').reduce((acc, curr) => acc + Number(curr.valor), 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                    </div>
                </div>
                <div className="rounded-xl border bg-card text-card-foreground shadow p-6">
                    <div className="flex flex-row items-center justify-between space-y-0 pb-2">
                        ...
                        {loading ? (
                            ...
                            data.map((item) => (
                                <TableRow key={item.id}>
                                    ...
                                    <TableCell>
                                        <Badge variant={item.status === 'Pago' ? 'default' : item.status === 'Atrasado' ? 'destructive' : 'secondary'}>
                                            {item.status}
                                        </Badge>
                                    </TableCell>
